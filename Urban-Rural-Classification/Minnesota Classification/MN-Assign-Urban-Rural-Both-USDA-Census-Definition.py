...
def assigned_or_unassigned(row):
    ...
    if pandas.notna(row['census_block_group']) and any(char.isdigit() for char in str(row['census_block_group'])):
        print('Assigned')
        return 'Assigned'
    else:
        print('Unassigned')
...
df['census_tract'] = df['census_tract'].apply(add_zeros)
...
def combine_columns(row):
    return str(row['census_block_group']) + row['census_tract']
